...
class Collector {
 ...
 public:
    ~Collector() {
        ...
        //              std::list<ObjectHandle<T>*>::iterator end2 = objlist.end();
        //              for (i= objlist.begin(); i!=end2; ++i) {
...
{
    if (mxGetClassID(handle) != mxUINT64_CLASS || mxIsComplex(handle) || mxGetM(handle) != 1 || mxGetN(handle) != 1)
