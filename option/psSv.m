function [svL, path, fold] = psSv(option, varargin)
...
%   modify    -  Feng Zhou (zhfe99@gmail.com), 06-26-2013
...
if isempty(subx1) && isempty(subx2)
    subx = '';
    if strcmp(type, 'fold')
        path = sprintf('%s/save/%s/%s', footpath, fold, prex);
    else
    ...
    if strcmp(type, 'fold')
        path = sprintf('%s/save/%s/%s_%s', footpath, fold, prex, subx);
    else
