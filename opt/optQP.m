function x = optQP(alg, H, f, AEq, bEq, A, b, blx, bux, x0)
% A wrapper of quadratic programming.
% It is an interface for calling different QP solvers, e.g., Matlab, CVX, Mosek.
%
% Math
...
%
% Input
%   alg     -  QP solver's name, 'matlab' | 'cvx' | 'mosek'
...
if strcmp(alg, 'matlab')
    ...
    x = quadprog(H, f, A, b, AEq, bEq, blx, bux, x0, option);
    warning on;
...
elseif strcmp(alg, 'mosek')
    ...
    res = mskqpopt(H, f, A, [], b, blx, bux, parMsk, cmd);
    x = res.sol.itr.xx;
