function [Vs, mes, V0s] = mcca(Xs, par)
...
%                'y': input needed to be centralized
%                'n': input has been centralized
...
% remove the last row (1s) if input is using homogeneous coordinate
if isHomo
    ds = ds - 1;
    for j = 1 : m
        Xs{j}(end, :) = [];
...
% core implementation
V0s = mccaCore(Xs, ds, lams, d);

% using homogeneous coordinate in output
Vs = cell(1, m);
for j = 1 : m
    Vs{j} = [V0s{j}, zeros(ds(j), 1); ...
             -mes{j}' * V0s{j}, 1];
end
...
%              Xs{i} is not using homogeneous coordinate
%              Xs{i} has been centralized
%   ds    -  dimension, 1 x m
